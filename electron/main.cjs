...
function createWindow() {
    ...
    // Set Content Security Policy to prevent unsafe-eval warning
        ...
        callback({
            responseHeaders: {
                ...
                'Content-Security-Policy': [
                    ...
                    "connect-src 'self' ws://localhost:* wss://localhost:* http://localhost:* https://localhost:* https://api.github.com; " +
                    "font-src 'self' data:;"
                ]
            }
        });
    ...
    console.log('Loading index page:', indexPath);
    mainWindow.loadFile(indexPath);

    // Open DevTools based on environment variables
    const shouldOpenDevTools = process.env.AUTO_OPEN_DEV_TOOLS === 'true' || process.env.ENABLE_DEV_TOOLS === 'true';
    const devToolsMode = process.env.DEV_TOOLS_MODE || 'detach';
    
    if (shouldOpenDevTools) {
        console.log('Opening DevTools in', devToolsMode, 'mode');
